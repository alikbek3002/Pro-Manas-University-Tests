import { BookOpen, FlaskConical, ChevronLeft, Check, X, ChevronDown, ChevronUp, Clock3, History } from 'lucide-react';
...
function subjectLabel(subject: string | null, language: 'ru' | 'kg' | undefined): string {
  ...
  switch (subject) {
    ...
    case 'kyrgyz': return kg ? 'Кыргыз тили' : 'Кыргызский';
...
export default function TestHistoryPage() {
  ...
  const historyQuery = useQuery(
    studentId && token
      ? testHistoryQueryOptions(studentId, token)
      : {
  ...
  const detailQuery = useQuery(
    studentId && token && expandedEntry
      ? testHistoryDetailQueryOptions(studentId, token, expandedEntry.id, expandedEntry.type)
      : {
      ...
      <div className="border-b-2 border-stone-100">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 py-4 flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-2 text-stone-500 hover:text-stone-900 transition-colors font-medium text-sm border-2 border-stone-200 hover:border-stone-400 px-3 sm:px-4 py-2 rounded-full"
          >
            <ChevronLeft className="h-4 w-4" />
            <span>{localizeUi(student?.language, 'Назад', 'Артка')}</span>
          </button>
        </div>
      </div>
