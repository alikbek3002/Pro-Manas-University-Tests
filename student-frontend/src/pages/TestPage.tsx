...
import {
  ...
  saveActiveTestSnapshot,
} from '../lib/activeTestStorage';
import {
...
function clampQuestionIndex(index: number, totalQuestions: number) {
  if (totalQuestions <= 0) {
    return 0;
  }
...
export default function TestPage() {
  const { student, token, logout, hasHydrated } = useAuthStore();
  ...
  const navigatedTestData = location.state?.testData as GeneratedTestResponse | undefined;
  const routeMatchedTestData = navigatedTestData && (!routeSessionId || navigatedTestData.test_session_id === routeSessionId)
    ? navigatedTestData
    : undefined;
  const persistedSnapshot = student ? loadActiveTestSnapshot(routeSessionId ?? null, student.id) : null;
  const testData = routeMatchedTestData ?? persistedSnapshot?.testData;
  const restoredProgress = persistedSnapshot?.progress;
  const initialQuestionCount = testData?.questions.length ?? 0;

  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(() =>
    clampQuestionIndex(restoredProgress?.currentQuestionIndex ?? 0, initialQuestionCount),
  );
  const [selectedAnswers, setSelectedAnswers] = useState<Record<string, number>>(
    () => restoredProgress?.selectedAnswers ?? {},
  );
  const [revealedAnswers, setRevealedAnswers] = useState<RevealState>(
    () => restoredProgress?.revealedAnswers ?? {},
  );
  ...
  const [bypassedFullscreen, setBypassedFullscreen] = useState(() => restoredProgress?.bypassedFullscreen ?? false);
  const [tabSwitchCount, setTabSwitchCount] = useState(() => restoredProgress?.tabSwitchCount ?? 0);
  ...
  const enterFullscreen = useCallback(async () => {
    try {
      ...
      // Fullscreen may not be available on some mobile browsers.
    }
  }, []);
  ...
  const exitFullscreen = useCallback(async () => {
    try {
      ...
      // Ignore fullscreen exit issues on unsupported browsers.
    }
  }, []);
  ...
  const handleScreenshotViolation = useCallback(async () => {
    ...
    try {
      const result = await reportScreenshotViolation();
  ...
  const testType = testData?.test_info.type ?? 'MAIN';
  ...
  useEffect(() => {
    ...
    const preventShortcuts = (e: KeyboardEvent) => {
      ...
      // Mac only: Cmd+Shift = screenshot shortcuts (Cmd+Shift+3/4/5)
      // On Windows, Win+Shift is used for language switching — do NOT trigger!
      ...
      if (!isMac && e.ctrlKey && e.shiftKey && normalizedKey === 's') {
        e.preventDefault();
        handleScreenshotViolation();
        return;
      }
  ...
  useEffect(() => {
    ...
    const handleFullscreenChange = () => {
      const fullscreenDocument = document as FullscreenCapableDocument;
      const isFull = !!(fullscreenDocument.fullscreenElement || fullscreenDocument.webkitFullscreenElement);
  ...
  useEffect(() => {
    if (!student || !testData || submitResult) {
      return;
    }
  ...
  }, [student, testData, submitResult, currentQuestionIndex, selectedAnswers, revealedAnswers, tabSwitchCount, bypassedFullscreen]);
  ...
  const totalQuestions = testData?.total_questions ?? 0;
  const isLastQuestion = currentQuestionIndex === totalQuestions - 1;
  const isFirstQuestion = currentQuestionIndex === 0;

  const unansweredIndices = useMemo(() => {
    if (!testData) return [] as number[];
    return testData.questions
      .map((q, i) => (revealedAnswers[q.id] ? null : i))
      .filter((i): i is number => i !== null);
  }, [testData, revealedAnswers]);
  ...
  const handleAnswerSelect = async (selectedIndex: number) => {
    ...
    try {
      const reveal = await answerStudentQuestion({
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await submitStudentTest({
        test_session_id: testData.test_session_id,
        type: testType,
      });
      ...
      clearActiveTestSnapshot(testData.test_session_id, student.id);
  ...
  return (
    ...
    >
      ...
      {showUnansweredModal && (
        <div
          className="fixed inset-0 z-[20000] flex items-center justify-center bg-black/60 backdrop-blur-sm"
          onClick={() => setShowUnansweredModal(false)}
        >
          <div
            className="mx-4 w-full max-w-md rounded-3xl bg-white p-6 sm:p-8 shadow-2xl relative z-[20001]"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="mx-auto inline-flex h-12 w-12 items-center justify-center rounded-full bg-amber-100 text-amber-600">
              <AlertCircle className="h-6 w-6" />
            </div>
            <h3 className="mt-4 text-center text-lg font-bold text-slate-900">
              {localizeUi(student?.language, 'Остались без ответа', 'Жоопсуз калды')}
            </h3>
            <p className="mt-2 text-center text-sm text-slate-500">
              {localizeUi(
                student?.language,
                'Ответьте на эти вопросы, прежде чем завершить тест. Нажмите номер, чтобы перейти.',
                'Тестти аяктаардан мурун бул суроолорго жооп бериңиз. Өтүү үчүн номерди басыңыз.',
              )}
            </p>
            ...
            <div className="mt-5 flex flex-wrap justify-center gap-2">
              {unansweredIndices.map((idx) => (
                <button
                  ...
                  className="inline-flex h-10 w-10 items-center justify-center rounded-xl border-2 border-stone-200 bg-white text-sm font-bold text-stone-700 transition-colors hover:border-black hover:bg-stone-50 active:scale-[0.97]"
                >
                  {idx + 1}
                </button>
              ))}
            ...
            <button
              type="button"
              onClick={() => setShowUnansweredModal(false)}
              className="mt-6 w-full h-11 rounded-xl border-2 border-stone-200 text-sm font-bold text-stone-700 hover:bg-stone-50 transition-colors"
            >
              {localizeUi(student?.language, 'Закрыть', 'Жабуу')}
            </button>
          </div>
        </div>
      )}
